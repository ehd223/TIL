...
Stackoverflow에서 가장 많이 나왔던 답변은 바로 ResponseEntity를 활용하는 것이었다. ResponseEntity 클래스는 HttpEntity를 상속받은 클래스로, RESTful 한 응답을 만들때 자주 사용된다. 일반적으로 데이터 값만 응답할 때 @ResponseBody를 사용하고는 하는데, 컨트롤러에서 ResponseEntity를 리턴타입으로 가지면 헤더, 바디, 상태코드를 포함하고 있어 조금 더 견고하고 안정적인 API를 짤 수 있다.
